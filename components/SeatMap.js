...
export default function SeatMap({ sessionId, cinemaId, typeColor }) {
  ...
  // Auto-fetch on mount / sessionId change — no manual open needed
  useEffect(() => {
    if (!sessionId) return
  ...
  return (
    <div onClick={e => e.stopPropagation()} style={{
      ...
      {loading && (
        <div style={{ textAlign:'center', padding:'24px 0', fontFamily:MONO, fontSize:11,
      ...
      {error && !loading && (
        <div style={{ fontFamily:MONO, fontSize:10, color:'#FF5757', letterSpacing:.5,
          textAlign:'center', padding:'12px 0' }}>{error}</div>
      )}
...
function SeatMapContent({ data, col }) {
  ...
  useEffect(() => {
    const calc = () => {
      ...
      setScale(gridW > containerW ? containerW / gridW : 1)
    }
    calc()
  ...
  // Actual rendered height after scaling (to collapse whitespace below scaled grid)
    ...
    <div>
      ...
      <div style={{ marginBottom:14 }}>
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
          ...
          <span className="occ-badge" style={{
            fontFamily:MONO, fontSize:9.5, fontWeight:700, color:barColor, letterSpacing:.5,
        ...
        <div style={{
          ...
          <div ref={gridRef} style={{ display:'inline-block' }}>
            ...
            <div style={{ textAlign:'center', marginBottom:10 }}>
              ...
              <div style={{ display:'inline-block', width:'70%', height:2,
                background:'linear-gradient(90deg,transparent,rgba(255,255,255,0.30),transparent)', borderRadius:2 }} />
            </div>

            {/* Rows */}
            <div style={{ display:'flex', flexDirection:'column', gap:3, alignItems:'center' }}>
              {rows.map((row, rowIdx) => (
                <div key={row.name} style={{ display:'flex', alignItems:'center', gap:3, animation:'rowFadeIn 0.2s ease ' + (rowIdx * 15) + 'ms both' }}>
                  <span style={{ fontFamily:MONO, fontSize:8, color:'rgba(255,255,255,0.30)', width:12, textAlign:'right', flexShrink:0 }}>{row.name}</span>
                  <div style={{ display:'flex', gap:2 }}>
                    {row.seats.map((seat, i) => <SeatDot key={i} seat={seat} />)}
                  </div>
                  <span style={{ fontFamily:MONO, fontSize:8, color:'rgba(255,255,255,0.30)', width:12, flexShrink:0 }}>{row.name}</span>
                </div>
              ))}
            </div>
          </div>
...
function SeatDot({ seat }) {
  if (seat.type === 'gap') return <div style={{ width:13, height:13 }} />
    ...
    return (
      <div title={`${seat.name}${seat.sold ? ' (Sold)' : ''}`}
        style={{ width:13, height:13, borderRadius:2,
          ...
          justifyContent:'center', fontSize:7, lineHeight:1 }}>♿</div>
    )
  }
  const bg     = seat.sold ? 'rgba(255,107,53,0.80)' : seat.unavailable ? 'rgba(255,255,255,0.06)' : 'rgba(255,255,255,0.22)'
  const border = seat.sold ? '#FF6B35'                : seat.unavailable ? 'rgba(255,255,255,0.08)' : 'rgba(255,255,255,0.30)'
  return (
    <div className="seat-dot"
      title={`${seat.name}${seat.sold?' (Sold)':seat.unavailable?' (Unavailable)':' (Available)'}`}
      style={{ width:13, height:13, borderRadius:2, background:bg, border:`0.5px solid ${border}` }} />
  )
}
