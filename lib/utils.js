...
export function getRuntime(movieId, movieMap = {}) {
  const raw = movieMap[movieId]?.runtime || KNOWN_MOVIES[movieId]?.runtime || 0
  // HOYTS API returns runtime as {minutes: N} object
  if (typeof raw === 'object') return raw?.minutes || raw?.value || 0
  return Number(raw) || 0
}
...
export function cleanMovieName(name) {
  ...
  return name
    ...
    .replace(/\s*:\s*\d{4}\s*re-release/gi, '')  // strip ': 2026 Re-release' style
    .replace(/\s*\[[^\]]*\]/g, '')
...
export function groupByDateAndHall(sessions, movieMap = {}, cinemaId = 'EGDENS') {
  ...
  sessions.forEach(s => {
    ...
    byDate[dateKey][hallName].sessions.push({
      ...
      posterImage: movieMap[movieId]?.posterImage || null,
      movieId,
      startMin,
      endMin,
      runtime,
